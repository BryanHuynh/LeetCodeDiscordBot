...
export const leetcodeAcDiscordMessageJob = async (dbService: ISubscriptionService, client: Client) => {
	...
	for (const leetcode_id in leetcode_id_acs) {
		...
		for (const ac_index in leetcode_id_acs[leetcode_id]) {
			...
			const leetcode_submission_builder: LeetCodeSubmissionBuilder = await new LeetCodeSubmissionBuilder().buildSubmissionFromServices(ac, leetcode_id, "");
			for (const subscription of subscriptions) {
				...
				try {
					const leetcode_submission = await leetcode_submission_builder.withDiscordName(subscription.discord_id).build();
					const thread = await createPrivateChannelRequest(guild, user, leetcode_submission);
					if (thread == null) return;
					const message = await createPrivateChannelMessage(thread, leetcode_submission);
